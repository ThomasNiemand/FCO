...
Note that the cutoff values for CFI and SRMR are higher (CFI = .971) and lower (SRMR = .036) than the respective empirical values (CFI = .960, SRMR = .038). That is, given the uncertainty of .05, as well as the present model and data, the cutoffs are close to the empirical values, but the model is marginally rejected. We can however simulate what happens if we assume less (.001) or more (.10) uncertainty.
...
## Visualizing the distribution of simulated fit indices

For a better understanding, we can plot the empirical values and cutoffs by taking values from the list generated by `gen_fit`. To obtain a better distribution, the replications are increased to 1,000 before plotting the distribution, density, and the cutoffs (.05: red, solid; .01: yellow, dashed; .001: green, dotted). Since all fit indices are stored in the `gen_fit` list object, this can easily be applied to other fit indices by changing the name "srmr" in the internal function `function(x) unname(x[c("srmr")]))`
...
The result found here demonstrates the consequences of uncertainty for the decision. When one is very conservative and assumes a high Type I error (.10), the cutoff (.036) is lower than the actual value (.038) and hence the present model should be rejected. When one is very lenient and feels safe with the model (.001), the cutoff (.040) is higher than the actual value and hence the model can be confirmed.
...
Mai, R., Niemand, T., & Kraus, S. (2021). A Tailor-Fit Model Evaluation Strategy for Better Decisions about Structural Equation Models, Technological Forecasting & Social Change (forthcoming).
